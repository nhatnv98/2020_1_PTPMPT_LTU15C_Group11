...
public class BannImpl extends UnicastRemoteObject implements Bank {
    ...
    public ArrayList<String> login(String cardNo, String PIN) throws RemoteException, SQLException {
        ...
        if (getCount() < 3) {
                    ...
                    try {
                        ...
                        if (rs.next()) {
                            ...
                            if (statuscheck == 0) {
                                ...
                                ps = MyConnection.getConnection().prepareStatement(sqlLogin);
            ...
            PreparedStatement ps;
            ps = MyConnection.getConnection().prepareStatement(sqlLock);
    ...
    public ArrayList<String> transfer(String cardNo1, String cardNo2, BigDecimal amount) throws RemoteException, SQLException {
        ...
        if (!cardNo1.isEmpty()) {
                    ...
                    if (amount.compareTo(new BigDecimal("50000")) == 0 || amount.compareTo(new BigDecimal("50000")) == 1) {
                        ...
                        if (amount.compareTo(acc1.getBalance()) == 1) {
                            ...
                            if (amount.remainder(new BigDecimal("5000")).equals(BigDecimal.ZERO)) {
                                ...
                                ps2 = MyConnection.getConnection().prepareStatement(sql2);
                                ps2.setBigDecimal(1, balance2);
                                ps2.setString(2, cardNo2);
                                ps2.executeUpdate();

                            } else {
                                code.add("Lỗi,số tiền phải là bội của 5000");
                            }
                        }

                    } else {
                        code.add("Chuyển khoản tối thiểu 50.000VND!");
                    }
            ...
            String sqlhistory = "INSERT INTO History(card_no1,card_no2,type,money_no,trans_date,code) VALUES(?,?,?,?,?,?)";
            ps = MyConnection.getConnection().prepareStatement(sqlhistory);
...
//        String value = getValue(cardNo);
    ...
    //check password
    public Boolean checkpass(String oldpass, String card_no) throws SQLException {
        String hashed = "";
        ...
        if (bCryptPasswordEncoder.matches(oldpass, hashed)) {
            return true;
        } else {
            return false;
        }
    ...
    private void changepassSQL(String card_no, String newpin) {
        BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
        String hashed = bCryptPasswordEncoder.encode(newpin);
        try {
            ...
            Logger.getLogger(BannImpl.class
